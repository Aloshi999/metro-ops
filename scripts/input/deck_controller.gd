...
## No mouse traps: mouse only wins after real pointer motion / click.
...
@export var cursor_offset_tiles: float = 7.0
@export var mouse_grab_seconds: float = 0.4
...
## Right-stick offset from camera center, in tiles (gamepad paint feel).
...
func _update_look() -> void:
	...
	if look.length() > look_deadzone:
		...
		# Vertical bias → zoom; horizontal + mild vertical → cursor offset
		zoom_delta -= shaped.y * 1.15
		cursor_offset = Vector2(shaped.x, shaped.y * 0.55) * cursor_offset_tiles
	else:
		cursor_offset = cursor_offset.lerp(Vector2.ZERO, 0.35)

	zoom_delta = clampf(zoom_delta, -1.75, 1.75) * zoom_speed
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("radial"):
		# Tap toggles; joypad hold releases confirm below
...
func pan_speed_at_zoom(zoom: float) -> float:
	return pan_speed / maxf(zoom, 0.35)
